...
export default function KuranPage() {
  ...
  const handleSurahSelection = (surahId: number, verseNumber?: number) => {
    setViewMode("mushaf")
    console.log(`Selected surah: ${surahId}, verse: ${verseNumber || 1}`)
  }
            ...
            <TabsContent value="mushaf" className="mt-0">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className={`p-4 bg-white dark:bg-gray-800 rounded-lg shadow ${isDesktop ? 'block' : 'hidden'}`}>
                  <h3 className="font-bold mb-3">Sayfa Seçimi</h3>
                ...
                {/* Quran content placeholder */}
                <div className={`${isDesktop ? 'col-span-2' : 'col-span-1'} bg-white dark:bg-gray-800 rounded-lg shadow p-4`}>
                  <div className="flex justify-center items-center h-64">
                    <div className="text-center">
                      <h3 className="text-xl font-bold mb-4">Açık Kuran API ile entegre edilmiştir</h3>
                      <p>Sayfa: {currentPage}</p>
                      <p>Çeviri: {translations.find(t => t.id === selectedTranslation)?.name}</p>
                      <p>Okuyucu: {qaris.find(q => q.id === selectedQari)?.name}</p>
                    </div>
                  </div>
                </div>
              </div>
